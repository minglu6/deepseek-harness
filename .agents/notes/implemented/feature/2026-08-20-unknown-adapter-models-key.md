...
`llm-qoder` is a third curated family: PAT, optional VPC instance, and the same model catalog editor as DeepSeek. It does not show gateway or OpenAPI URLs. The PAT stores under the derived route reference (`QODER_CN_API_KEY` for `qoder-cn`), not a launch-environment Qoder token, so the Models field stays writable. Other unknown adapters stay key-only.
...
**Bind `qoder-cn` to `QODERCN_PERSONAL_ACCESS_TOKEN`.** Rejected: a launch-environment Qoder token then locks the Models field and shadows a typed PAT.

## Consequences

Any adapter that declares a settings path can be given a key from Models. Extra fields (VPC, custom endpoints) remain yaml. A schema without `apiKeyEnv` still materializes a user profile so the row becomes configured and deletable. The qoder PAT is the page-stored `QODER_CN_API_KEY`; a launch-environment Qoder token is not the Models credential.

## Testing

`packages/client/ui-settings-models/tests/components.client.spec.tsx` covers Add-list order, the unknown-family key field, storing `PLAIN_API_KEY` while materializing the profile, and a launch-environment Qoder token leaving the `qoder-cn` field writable under `QODER_CN_API_KEY`.
